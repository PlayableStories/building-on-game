 ...
 */
const AROUND = ['B3', 'D3', 'A3', 'E3'] as const;
...
const FABRIC: Neighbour[] = [{ kind: 'fabric', cell: 'B3' }];
...
describe('§8.6 resolution order', () => {
  it('1. an explicit pair wins', () => {
    ...
    expect(lineFor(writing, plan('home-farm'), 'C4', beside('kitchen'))).toBe(
      'A short walk with wet hands. This is the version that gets used.',
    );
  });

  it('matches a pair in either direction', () => {
    const a = lineFor(writing, plan('home-farm'), 'C3', beside('bedroom'));
    const b = lineFor(writing, plan('bedroom'), 'C3', beside('home-farm'));
  ...
  it('2. a quality match wins when no pair is written', () => {
    ...
    expect(lineFor(writing, plan('heat-pump'), 'C3', beside('study'))).toBe(
  ...
  it('fires a quality match in either direction', () => {
    ...
    expect(lineFor(writing, plan('bin-store'), 'C3', beside('study'))).toBe(
      'You will know what was cooked, and for how long, some hours later.',
    );
    // ...and what the neighbour does to the new plan.
    expect(lineFor(writing, plan('study'), 'C3', beside('bin-store'))).toBe(
      'You will know what was cooked, and for how long, some hours later.',
    );
  });
  ...
  it('takes the strongest quality when several fire', () => {
    ...
    const line = lineFor(writing, plan('bedroom'), 'C3', beside('gym'));
  ...
  it('3. orientation fires when nothing else does', () => {
    expect(lineFor(writing, plan('solar-array'), 'C1', [])).toBe(
      'A lovely gesture. Very little electricity.',
    );
    expect(lineFor(writing, plan('home-farm'), 'B4', [])).toBe(
  ...
  it('4. silence is a valid result', () => {
    ...
    expect(lineFor(writing, plan('shed'), 'B4', [])).toBeNull();
    expect(lineFor(writing, plan('shed'), 'B5', beside('hall'))).toBeNull();
  });
...
describe('what caused the line (§8.6)', () => {
  it('names the placement and the neighbour, for a pair', () => {
    const result = observationFor(writing, plan('home-farm'), 'C4', beside('kitchen'));
    expect(result?.kind).toBe('pair');
    expect(result?.cause).toBe('Home farm beside Kitchen');
  });

  it('names them for a quality match too', () => {
    const result = observationFor(writing, plan('heat-pump'), 'C3', beside('study'));
  ...
  it('calls the old house by a name rather than a grid reference', () => {
    const result = observationFor(writing, plan('wall-insulation'), 'C3', FABRIC);
  ...
  it('names where it is standing for an orientation line, since nothing caused it', () => {
    ...
    const south = observationFor(writing, plan('glass-extension'), 'C3', []);
  ...
  it('tells the street apart from the strip the house shadows', () => {
    const street = observationFor(writing, plan('terrace'), 'C1', []);
    const shadow = observationFor(writing, plan('terrace'), 'C4', []);
  ...
  it('lights the placement and the neighbour that fired', () => {
    const result = observationFor(writing, plan('home-farm'), 'C4', beside('kitchen'));
    expect(result?.cell).toBe('C4');
    expect(result?.because).toEqual(['B3']);
  });
  ...
  it('lights every neighbour that fired, not just the first', () => {
    ...
    expect(result?.because).toEqual(['B3', 'D3']);
  ...
  it('lights only the neighbour that caused the quality that won', () => {
    ...
    const result = observationFor(writing, plan('bedroom'), 'C3', beside('gym', 'shed'));
    expect(result?.kind).toBe('quality');
    expect(result?.because).toEqual(['B3']);
...
describe('orientation', () => {
  ...
  it('says nothing in row 3, which is neither street nor garden', () => {
    expect(lineFor(writing, plan('solar-array'), 'C3', [])).toBeNull();
  });

  it('distinguishes the two ends of the plot', () => {
    const north = lineFor(writing, plan('glass-extension'), 'C1', []);
    const south = lineFor(writing, plan('glass-extension'), 'C5', []);
    expect(north).toBe(
...
describe('§8.7 pairings against something other than a plan', () => {
  it('speaks up for air conditioning beside anything at all', () => {
    ...
    expect(lineFor(writing, plan('air-conditioning'), 'C3', beside('shed'))).toBe(line);
    expect(lineFor(writing, plan('air-conditioning'), 'C3', beside('lawn'))).toBe(line);
    expect(lineFor(writing, plan('air-conditioning'), 'C3', FABRIC)).toBe(line);
  });

  it('says nothing for air conditioning with no neighbour at all', () => {
    // "Beside anything" still needs something to be beside.
    expect(lineFor(writing, plan('air-conditioning'), 'C3', [])).toBeNull();
  });

  it('reacts to the old solid walls, not to a room', () => {
    expect(lineFor(writing, plan('wall-insulation'), 'C3', FABRIC)).toBe(
      'Warmer. And a damp risk you will be managing for a decade.',
    );
    expect(lineFor(writing, plan('wall-insulation'), 'C3', beside('shed'))).toBeNull();
  });
  ...
  it('prefers a line naming both plans over a wildcard one', () => {
    ...
    expect(specific?.because).toEqual(['B3']);
  });
});
